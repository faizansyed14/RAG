...
def outline_to_dict_tree(outline_node_list: list[OutlineNode], total_pages: int) -> list[dict]:
    """Convert the outline tree directly to the RAG JSON shape. Preserves the natural outline nesting without font-overlay rewriting. """
    ...
    # Stable DFS pre-order node ids, zero-padded to 4 (tree-node-id convention;
