"""RAG engine SDK client (vendored): the 0.2.x cloud surface, now with a local mode."""
...
# The two citation tag formats the chat model writes and renders.
...
def _env_cloud_key(spelling: str, inline: str = "api_key=...") -> str:
    ...
    if not key:
        raise RagEngineError(
            f"{spelling} reads the cloud API key from the "
...
# there as a RagEngineError — never later, never silently.
...
def _resolve_index_slot(index) -> "tuple[_CloudKey, dict[str, Any]]":
    ...
    if isinstance(index, Mapping):
        ...
        return None, {name: value for name, value in mapped.items()
                      if value is not None}
    raise RagEngineError("index must be a string or a dict.")
...
def _resolve_chat_slot(chat) -> "tuple[Optional[str], dict[str, Any]]":
    ...
    if isinstance(chat, Mapping):
        ...
        return "own", {name: value for name, value in mapped.items()
                       if value is not None}
    raise RagEngineError("chat must be a string or a dict.")


class RagEngineClient:
    """
    Python SDK client for local-mode RAG document indexing and chat.

    Two independent sides, each locally run or cloud-managed:

    - **index** — where documents live. With an ``api_key`` they live in
      your cloud account, indexed by the managed pipeline,
    ...
    Usage:
        client = RagEngineClient()                  # local docs + your model
        client = RagEngineClient(api_key="...")     # cloud docs + managed chat
        client = RagEngineClient(api_key="...",     # cloud docs + your model
    ...
    only when the code explicitly says cloud; a bare ``RagEngineClient()``
    stays local regardless of the environment).

    Args:
        api_key (str, optional): cloud API key
    ...
    RagEngineCloudClient / RagEngineLocalClient pin the index side at
    ...
    def __init__(
        ...
        *,
        ...
        if api_key == "":
            raise RagEngineError(
                "api_key is an empty string. Pass a real cloud API key for "
        ...
        if index is not None:
            ...
            if declared == "local" and cloud_key is not None:
                raise RagEngineError(
                    ...
                    "cloud documents. Drop it, or use RagEngineCloudClient."
                    if pinned else
        ...
        if cloud_key is not None:
            if index_conf:
                raise RagEngineError(
                    "Cloud documents are indexed by the cloud pipeline "
            ...
            if chat_mode == "managed":
                if pinned:
                    exits = (f"{pinned} pins local documents: use "
                             "RagEngineCloudClient (or RagEngineClient("
                             "api_key=...))")
                else:
            ...
            self.chat_model = opt.chat_model
            self.chat_backend = chat_conf.get("chat_backend")
            self.storage_path = index_conf.get("storage_path") or ".rag-engine"
    ...
    def _wait_until_ready(self, doc_id: str, timeout: float = 1800.0) -> None:
        ...
        while True:
            try:
                ...
                poll_failures = 0
            except (RagEngineError, requests.RequestException) as exc:
    ...
    def get_block(self, doc_id: str, block_id: str) -> dict[str, Any]:
        ...
        blocks, so local mode raises RagEngineError.
        ...
        Returns:
            ...
            height (0-1000), origin top-left. RagEngineError with
    ...
    def submit_query(self, doc_id: str, query: str, thinking: bool = False) -> dict[str, Any]:
        ...
        RagEngineError. Use ``chat()`` instead.
        """
        return self._require_cloud(
        ...
        *,
        ...
        Args:
            ...
            show_process: Answer lane, streamed chat — weave the run into
                ...
                ``ChatProcessOptions``) selects the parts —
            ...
            citations: Own-model chat: cite every claim the way the chat model
                chat does — ``<cite doc="…" page="…"/>`` tags, ``block="…"``
                added where the cloud document has blocks. The guidance is
                the cloud MCP server's ``cited_answer`` prompt, joining
    ...
    def get_document_id(self, name: str) -> str:
        ...
        Raises RagEngineError if no document with that name exists.
        """
        ...
        if docs:
            return docs[0]["id"]
        raise RagEngineError(f"No document named {name!r} found.")
    ...
    def agent_tools(
        ...
        Cloud: the full live read tool set, discovered from the cloud
        ...
        executed from your process over MCP. Raises RagEngineError if the
        ...
        USAGE_LIMIT_REACHED tool error, which raise RagEngineError.
    ...
    def as_openai_tools(self, include_management: bool = False,
        ...
        "Bearer <your cloud API key>"}}`` (drop ``?tools=read`` for
    ...
    def as_anthropic_tools(self, include_management: bool = False,
        ...
        RagEngineError, which the tool runner flattens into an
        is_error result.

        Cloud: the full live read tool set (search, folders, images — as
        enabled for your key), discovered from the cloud MCP server
        ...
        "rag", "url": f"{BASE_URL}/mcp?tools=read",
        "authorization_token": <your cloud API key>}]`` (drop
    ...
    def as_claude_mcp(self, include_management: bool = False, *,
        ...
        Cloud: returns the remote cloud MCP config.
        ...
        call)::
            ...
            options = ClaudeAgentOptions(
                ...
                mcp_servers={"rag": client.as_claude_mcp()},
                # Pre-approval only — the server itself is already gated.
                allowed_tools=["mcp__rag"],
            )
        """
    ...
    def agent_instructions(self, *, include_management: bool = False) -> str:
        ...
        Cloud: the live instructions the cloud MCP server serves for
        your key's tool set, fetched over the same session as
        ``agent_tools()`` — server-side guidance updates arrive without an
        SDK release. Raises RagEngineError if the server cannot be
    ...
    def document_context(self, doc_id: Union[str, list[str]]) -> str:
        ...
        RagEngineError if a document does not exist.
        """
        from .agent_tools import doc_targeting_block
        if doc_id is None:
            raise RagEngineError("doc_id must be a string or a list of "
                                    "strings.")
    ...
    def citation_prompt(self, format: str = "cite") -> str:
        ...
        how each citation is written — as served by the cloud MCP
        ...
        ``<cite doc= page= block=/>`` tags the chat model writes and
    ...
    def get_citations(
        ...
        and the block's ``text``. Reads both tag formats the chat model
        ...
        Args:
            ...
            doc_id (str | list[str], optional): The documents the answer
                ...
                raise RagEngineError naming both ids: pass ``doc_id``
                to pick.
    ...
    def resolve_citations(
        ...
        become ``[[1]](#rag-citation-01)``, one number per distinct
    ...
    def folder_context(self, folder_id: str) -> str:
        ...
        places nothing for it. Raises RagEngineError if the folder does
        not exist.
        """
        from .agent_tools import folder_targeting_block
        if folder_id is None:
            raise RagEngineError("folder_id must be a string.")
    ...
    def list_folders(self, parent_folder_id: Optional[str] = None) -> dict[str, Any]:
        """
        List folders. Cloud-only: local mode raises RagEngineError.
        """
        return self._require_cloud(
    ...
    def get_folder_path(self, folder_id: str) -> str:
        ...
        RagEngineError if the folder does not exist.
        """
        self._require_cloud(
        ...
        if path is None:
            raise RagEngineError(f"Folder {folder_id!r} not found.")
        return path
    ...
    def get_folder_id(self, path: str) -> str:
        ...
        Cloud-only. Raises RagEngineError if no folder, or more than
        one, has that path.
        """
        self._require_cloud(
...
class RagEngineCloudClient(RagEngineClient):
    """Cloud mode — the class name says cloud, so the key may come from
    the environment: ``RagEngineCloudClient()`` reads PAGEINDEX_API_KEY.
    ...
    def __init__(
        ...
        *,
        ...
        if index is None:
            ...
            if not api_key:
                raise RagEngineError(
                    "RagEngineCloudClient requires a cloud API key — "
...
class RagEngineLocalClient(RagEngineClient):
