"""Cloud mode of the RAG engine SDK (vendored), based on the 0.2.8 client."""
import requests
...
class CloudAPI:
    """
    Python SDK client for the cloud API.
    """
    ...
    def get_page_image(self, doc_id: str, page: int) -> str:
        ...
        for img in response.json().get("images") or []:
            if img.get("page") == page and img.get("url"):
                return img["url"]
        raise RagEngineError(f"No image URL returned for page {page}.")
    ...
    def get_document_image(self, doc_id: str, img_id: str) -> str:
        ...
        if not url:
            raise RagEngineError(f"No image URL returned for {img_id!r}.")
        return url
    ...
    def submit_query(self, doc_id: str, query: str, thinking: bool = False) -> Dict[str, Any]:
        """
        Submit a retrieval query for a specific document.
    ...
    def chat_completions(
        ...
        """
        Cloud chat completions. Optionally scoped to specific documents.
