...
from .errors import RagEngineError, _rag_engine_cause
...
def _split_chat_messages(messages) -> "tuple[list[str], list[dict]]":
    ...
    if not messages:
        raise RagEngineError("messages must be a non-empty list.")
    ...
    if not history:
        raise RagEngineError("messages must contain a user or assistant "
                                "message.")
...
def _openai_model(protocol: str, model_name: str, backend=None):
    ...
    if protocol == "responses":
        ...
        sdk_client._rag_engine_caller_http = "http_client" in (backend or {})
...
def _openai_agent(client, protocol: str, model_name: str, instructions: str,
    ...
    try:
        ...
        raise RagEngineError(f"Invalid model settings: {exc}") from exc
    return Agent(
        name="RAG",
...
def _validate_max_turns(max_turns) -> None:
    if max_turns is not None and (not isinstance(max_turns, int)
                                  or max_turns < 1):
        raise RagEngineError("max_turns must be a positive integer.")
...
def _conversation_cache_key(model_name: str, instructions: str, doc_id,
    ...
    return "rag-" + hashlib.sha256(seed.encode()).hexdigest()[:16]


def _model_backend_error(exc, lane: str, client=None) -> RagEngineError:
    ...
    if (getattr(client, "api_key", None)
        ...
        message += (
            ...
            "provider credentials; the RagEngineClient api_key does not cover "
    ...
    return RagEngineError(message,
                             status_code=getattr(exc, "status_code", None))


def _translate_run_error(exc, max_turns, lane, client=None) -> RagEngineError:
    ...
    if isinstance(exc, AgentsException):
        cause = _rag_engine_cause(exc)
        if cause is not None:
            # a tool failure the invoker re-raised, wrapped on its way out
            return RagEngineError(str(cause), status_code=cause.status_code)
        return RagEngineError(f"The agent backend failed: {exc}")
...
async def _aclose_backend(agent) -> None:
    ...
    if getattr(backend, "_rag_engine_caller_http", False):
        return
...
def _wrap_max_turns(max_turns) -> RagEngineError:
...
def _process_options(show_process) -> dict:
    ...
    for key in ("thinking", "tool_call", "tool_result"):
        if not isinstance(options[key], bool):
            raise RagEngineError(f"show_process[{key!r}] must be a bool.")
...
def run_responses(client, input, model: Optional[str] = None,
    ...
    if isinstance(input, str) and input.strip():
        ...
        raise RagEngineError("messages must be a non-empty string or list "
                                "of item dicts.")
    scope = client._local_doc_scope(doc_id)
    block = targeting_block(client, doc_id, folder_id)
    conversation = items
...
def _anthropic_system(client, extra_system) -> list[dict]:
    ...
    raise RagEngineError("system must be a string or a list of blocks.")
...
def run_messages(client, messages, model: str,
    ...
    if (not isinstance(messages, list) or not messages
        ...
        raise RagEngineError("messages must be a non-empty string or a "
    ...
    try:
        ...
        if owns_transport:
            backend_client.close()
    if not turns:
        raise RagEngineError("The model returned no response.")
    captured: dict = {}
