...
the cloud MCP server from OpenAI's side — the read-only
...
def _tool_failure(ctx, error):
    """The framework's tool-failure formatter, narrowed: a RAG engine failure
    ...
    if _rag_engine_cause(error) is not None:
        raise error
